...
# -*- coding: utf-8 -*-
"""
Network Driver Exception Hierarchy

Defines structured error handling for network device operations with HTTP-compatible
status codes and standardized error messages. Enables proper error classification
and debugging across different device vendors and connection protocols.
"""
...
class LoginFailed(BaseError):
    def __init__(self, msg: str = "Login failed, please check your username and password.") -> None:
        super().__init__(msg)
...
class SessionInitFailed(BaseError):
    def __init__(self,
                 msg: str = "Session init failed, please check the device and credentials.") -> None:
        super().__init__(msg)
        self.status_code = 502 # Bad Gateway
