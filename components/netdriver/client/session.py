...
class Session:
    ...
    async def _consume_runconf_queue(self):
        ...
        output: str = ""
        while not self._is_closing:
            task_dict = await self._runconf_req_merger.get_mergeable_tasks()
            if not task_dict:
                continue
            for vsys, tasks in task_dict.items():
                ...
                try:
                    ...
                    self._runconf_req_merger.set_mergeable_tasks_result(tasks=tasks, output=output, exception=exception)
            
    async def _consume_routes_queue(self):
        self._logger.info(f"Start consuming routes queue")
        output: str = ""
        while not self._is_closing:
            task_dict = await self._route_req_merger.get_mergeable_tasks()
            if not task_dict:
                continue
            for vsys, tasks in task_dict.items():
                output = None
                ...
                try:
                    ...
                    self._route_req_merger.set_mergeable_tasks_result(tasks=tasks, output=output, exception=exception)

    async def _consume_hitcounts_queue(self):
        self._logger.info(f"Start cosuming hitcountrs queue")
        while not self._is_closing:
            task_dict = await self._hitcount_req_merger.get_mergeable_tasks()
            if not task_dict:
                continue
            output = None
            exception = None
            for vsys, tasks in task_dict.items():
                ...
                try:
                    ...
                    exception = PullConfigFailed(e, output=output)
                     ...
                     self._hitcount_req_merger.set_mergeable_tasks_result(tasks=tasks, output=output, exception=exception)
    ...
    async def send_cmd(self, command: str, vsys: str = None, mode: Mode = None,
        ...
        :param command: command to execute, support multi-line command
