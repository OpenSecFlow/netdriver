...
class SessionPool:
    """
    Connection pool for network device sessions to prevent resource exhaustion.
    
    Singleton pattern ensures only one pool exists to avoid connection leaks
    and duplicate sessions to the same device which could cause authentication lockouts.
    """
    _instance = None
    ...
    def __new__(cls, config: Configuration = None) -> "SessionPool":
        ...
        if not cls._instance:
            ...
            # Background monitoring prevents abandoned sessions from consuming device resources
    ...
    async def _get_session_by_key(self, session_key: str) -> Optional[Session]:
        """
        Validates session health and removes stale connections.
        
        Network devices often silently drop idle connections or enforce session timeouts.
        This validation prevents commands from being sent to dead sessions.
        """
    ...
    async def _remove_session_by_key(self, session_key: str) -> None:
        """Thread-safe session cleanup without closing the connection."""
    ...
    async def get_session(self,
        ...
        """
        Connection factory with reuse optimization and conflict detection.
        
        Session reuse prevents authentication rate limiting and reduces device load.
        Conflict detection ensures command execution safety by preventing simultaneous
        access to the same device with different parameters.  To get a session by key, if session is not exist or not same, create a new session.
        ...
        # Prevent configuration conflicts when same device accessed with different parameters
        if _session and not _session.is_same(
            ...
            if _session.is_idle is True:
                ...
                # Safety mechanism: prevent concurrent access with different configs
        ...
        # Async initialization may fail after session creation - handle gracefully
    ...
    async def _handle_closed_session(self, session: Session):
        """Cleanup with timeout protection to prevent hanging on unresponsive devices."""
        ...
        try:
            ...
            # Timeout prevents hanging on devices that don't respond to close commands
    ...
    async def _display_sessions_info(self):
        """Operational visibility for monitoring active connections."""
        table = []
    ...
    async def _remove_closed_sessions(self):
        # Snapshot prevents "dictionary changed size during iteration" race condition
    ...
    async def monitor_sessions(self) -> None:
        """Background task to prevent resource leaks from abandoned sessions."""
        while True:
